...
sys.path.insert(0, '/home/pi/dynamixel_hr/dxl')
...
def init():
    ...
    chain.goto(1, 0, speed=100)
    chain.goto(1, 4095, speed=100)
    chain.goto(1, 0, speed=100)
    return
...
def main():
    ...
    while True:  # start test
        ...
        if (NewCounter != 0):  # Counter has CHANGED
            Position = Position + NewCounter * abs(NewCounter)
            if Position > 360:
                Position = 360
            if Position < 1:
                Position = 1
            print(NewCounter, Position)  # some test print
            chain.goto(1, int(Position * 11.375), speed=100)
